...
export const bizApi = {
  ...
  supplierScorecard: () => request<any[]>('/biz/supplier-scorecard'),
  coqAnalysis: () => request<any>('/biz/coq-analysis'),
  contractPlanList: (contractNo: string) => request<any>(`/biz/contract-plan/list?contract_no=${encodeURIComponent(contractNo)}`),
  planGenerate: (body: { contract_no: string; terms: number; first_date?: string }) => request<any>('/biz/contract-plan/generate', { method: 'POST', body: JSON.stringify(body) }),
  planAdd: (body: { contract_no: string; due_date: string; plan_amount: number; remark?: string }) => request<any>('/biz/contract-plan/add', { method: 'POST', body: JSON.stringify(body) }),
  ...
  customer360: (code: string) => request<any>(`/biz/customer-360/${encodeURIComponent(code)}`),
  productionSchedule: () => request<any>('/biz/production-schedule'),
  budgetDashboard: (month?: string) => request<any>(`/biz/budget-dashboard?month=${encodeURIComponent(month || '')}`),
  budgetSet: (body: { department: string; budget_month: string; budget_amount: number; remark?: string }) => request<any>('/biz/budget-set', { method: 'POST', body: JSON.stringify(body) }),
  configList: () => request<any[]>('/biz/config/list'),
  configSet: (body: { config_key: string; config_value: string }) => request<any>('/biz/config/set', { method: 'POST', body: JSON.stringify(body) }),
  supplierDelivery: (code: string) => request<any>(`/biz/supplier-delivery/${encodeURIComponent(code)}`),
  inventoryAging: () => request<any>('/biz/inventory-aging'),
  approvalEfficiency: () => request<any>('/biz/approval-efficiency'),
  monthlyReport: (month?: string) => request<any>(`/biz/monthly-report?month=${encodeURIComponent(month || '')}`),
