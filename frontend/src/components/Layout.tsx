...
const ProductionTrackingPage = lazy(() => import('./ProductionTrackingPage'));
...
const BIZ_PAGES: Array<{ type: any; label: string; icon: string; roles: string[] }> = [
  ...
  { type: 'scorecard',      label: '供应商记分卡', icon: '🏅', roles: ['admin','procurement','accounting'] },
  { type: 'workbench',      label: '快捷工作台',   icon: '🧰', roles: ['admin','warehouse','sales','production','procurement'] },
  ...
  { type: 'schedule',       label: '生产排程看板', icon: '📅', roles: ['admin','production','warehouse'] },
  { type: 'customer360',    label: '客户360视图',  icon: '👤', roles: ['admin','sales','accounting'] },
  { type: 'budget',         label: '部门预算看板', icon: '🧮', roles: ['admin','accounting','hr'] },
...
export default function Layout() {
  ...
  const PAGE_ENTRIES: Array<{ label: string; icon: string; page: Page }> = [
    ...
    { label: '供应商记分卡', icon: '🏅', page: { type: 'scorecard' } },
    { label: '快捷工作台', icon: '🧰', page: { type: 'workbench' } },
    ...
    { label: '客户360视图', icon: '👤', page: { type: 'customer360' } },
    { label: '生产排程看板', icon: '📅', page: { type: 'schedule' } },
    { label: '部门预算看板', icon: '🧮', page: { type: 'budget' } },
  ...
  const getPageTitle = () => {
    ...
    if (page.type === 'norule') return '🔢 单据编号规则';
