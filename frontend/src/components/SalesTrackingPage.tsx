import { useEffect, useState } from 'react';
import { bizApi } from '../api';

const STAGES = ['已下单', '已审核', '已出库', '已回款'];

function StageBar({ stage, rcvProgress }: { stage: number; rcvProgress: number }) {
  return (
    <div className="min-w-[220px]">
      <div className="flex items-center gap-1">
        {STAGES.map((s, i) => (
          <div key={s} className="flex items-center gap-1 flex-1 last:flex-none">
            <div className={`w-4 h-4 rounded-full flex items-center justify-center text-[8px] font-bold shrink-0 ${i <= stage ? 'bg-indigo-500 text-white' : 'bg-slate-200 text-slate-400'}`}>
              {i <= stage ? '✓' : i + 1}
            </div>
            {i < STAGES.length - 1 && <div className={`h-0.5 flex-1 ${i < stage ? 'bg-indigo-400' : 'bg-slate-200'}`}></div>}
          </div>
        ))}
      </div>
      <div className="flex justify-between mt-1">
        {STAGES.map((s, i) => (
          <span key={s} className={`text-[9px] ${i <= stage ? 'text-indigo-600 font-medium' : 'text-slate-400'}`}>{s}</span>
        ))}
      </div>
      {stage >= 2 && (
        <div className="mt-1.5">
          <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
            <div className={`h-full rounded-full transition-all ${Number(rcvProgress) >= 100 ? 'bg-emerald-400' : 'bg-amber-400'}`} style={{ width: `${Math.min(100, Number(rcvProgress) || 0)}%` }}></div>
          </div>
          <p className="text-[9px] text-slate-400 mt-0.5">回款进度 {Number(rcvProgress) || 0}%</p>
        </div>
      )}
    </div>
  );
}

/** 销售订单执行跟踪：下单 → 审核 → 出库 → 回款 四段进度可视化 */
export default function SalesTrackingPage() {
  const [rows, setRows] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');

  useEffect(() => {
    bizApi.salesTracking().then(r => { setRows(r.data || []); setLoading(false); })
      .catch(e => { setError(e.message || '加载失败'); setLoading(false); });
  }, []);

  const stageCount = [0, 1, 2, 3].map(i => rows.filter(r => Number(r.stage) === i).length);

  if (error) return <div className="p-16 text-center"><div className="text-5xl mb-3">⚠️</div><p className="text-red-500">{error}</p></div>;

  return (
    <div className="erp-fade-in p-6 space-y-5 max-w-[1400px] mx-auto">
      <div>
        <h2 className="text-xl font-bold text-slate-800 tracking-tight">🚚 销售执行跟踪</h2>
        <p className="text-sm text-slate-400 mt-1">最近 25 张销售单的四段执行进度：下单 → 审核 → 出库 → 回款（回款进度实时取自应收核销）</p>
      </div>

      {/* 阶段分布 */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {STAGES.map((s, i) => (
          <div key={s} className="erp-card p-4 flex items-center gap-3">
            <span className={`w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold ${i === 0 ? 'bg-slate-100 text-slate-500' : i === 1 ? 'bg-blue-50 text-blue-600' : i === 2 ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'}`}>{i + 1}</span>
            <div>
              <p className="text-[11px] text-slate-400">处于「{s}」阶段</p>
              <p className="text-lg font-bold text-slate-800 tabular-nums">{loading ? '-' : stageCount[i]}</p>
            </div>
          </div>
        ))}
      </div>

      {/* 进度明细 */}
      <div className="erp-card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="erp-table">
            <thead><tr><th>销售单号</th><th>客户</th><th>日期</th><th className="text-right">金额</th><th>执行进度</th><th>状态</th></tr></thead>
            <tbody>
              {rows.map(r => (
                <tr key={r.sales_no}>
                  <td className="font-mono whitespace-nowrap text-indigo-600">{r.sales_no}</td>
                  <td className="whitespace-nowrap">{r.customer_name || '—'}</td>
                  <td className="whitespace-nowrap text-slate-500">{String(r.sales_date || '').slice(0, 10)}</td>
                  <td className="text-right tabular-nums whitespace-nowrap font-medium">¥{Number(r.total_amount || 0).toLocaleString()}</td>
                  <td><StageBar stage={Number(r.stage) || 0} rcvProgress={r.rcv_progress} /></td>
                  <td className="whitespace-nowrap">
                    <div className="flex flex-col gap-0.5 text-[10px]">
                      <span className="text-slate-500">单据 {r.sales_status || '—'}</span>
                      <span className={r.shipping_status === '已出库' ? 'text-emerald-600' : 'text-slate-400'}>发货 {r.shipping_status || '未发货'}</span>
                    </div>
                  </td>
                </tr>
              ))}
              {rows.length === 0 && !loading && (
                <tr><td colSpan={6} className="py-14 text-center">
                  <div className="text-4xl mb-3">🚚</div>
                  <p className="text-sm text-slate-400">暂无销售单 —— 发生销售业务后自动呈现执行进度</p>
                </td></tr>
              )}
            </tbody>
          </table>
        </div>
      </div>
    </div>
  );
}
