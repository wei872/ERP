            ...
            {data.map((row, idx) => <tr key={(row as any).id || idx}>
              ...
              <td className="text-center sticky right-0 bg-white" style={{ boxShadow: '-4px 0 8px -4px rgba(0,0,0,0.06)' }}>
                <div className="flex items-center justify-center gap-1.5">
                  ...
                  {tableKey === 'cust_customer_main' && <button onClick={() => window.dispatchEvent(new CustomEvent('erp:navigate', { detail: { type: 'customer360', code: String((row as any).customer_code || '') } }))} className="px-2.5 py-1 text-[11px] text-sky-700 bg-sky-50 hover:bg-sky-100 rounded-md transition-colors font-medium">👤 360</button>}
