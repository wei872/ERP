...
| 模块 | 页面 | 说明 |
|---|---|---|
| 📊 控制台 | Dashboard | 销售趋势 / 采购趋势 / 库存金额分布 / KPI 卡片（可配置显隐） |
| 🖥️ 经营驾驶舱 | BigScreen | 数据大屏投屏模式：60s 自动刷新，KPI/面板指标可配置，移动端可投屏 |
| 📦 盘点盈亏分析 | StocktakeAnalysisPage | 盘点差异月度趋势 / 仓库分布 / TOP 差异商品，与整仓批量盘点联动 |
| ⚖️ 采购比价评级 | PriceComparePage | 同物料多供应商历史价对比，最低价自动推荐 + 潜在节省测算 |
| 💎 销售提成计算 | CommissionPage | 目标达成率阶梯计提 → 审批 → 一键同步工资表（奖金+实发重算） |
| 📑 合同执行跟踪 | ContractTrackingPage | 合同↔订单↔收付款↔收款计划↔开票五单关联，执行/回款进度 + 到期/逾期自动预警 |
| 🚨 质量异常闭环 | NcrPage | NCR 发起→处置→复检→关闭状态机，联动批次冻结/报废出库 |
| 🏅 供应商记分卡 | SupplierScorecardPage | 交期40%+质量40%+价格20% 加权评分，A~D 星级榜单 |
| 🧰 快捷工作台 | MobileWorkbenchPage | 移动端快捷盘点/入库/出库 + 常用入口直达 |
| 📣 催收管理中心 | CollectionPage | 逾期应收账龄四桶看板 + 催收记录留痕 + 催款函一键打印 |
| 🏭 工单成本分析 | WorkOrderCostPage | 领料/人工/制费三维成本 + 工单损益 + 超计划用料预警 |
| 🔮 经营预测 | ForecastPage | 加权均值+线性趋势双模型预测下月销售，与目标联动预警 |
| 👤 客户360视图 | Customer360Page | 档案/信用/订单/应收/合同/催收一屏聚合，客户行一键直达 |
| 📅 生产排程看板 | ProductionSchedulePage | 工单甘特排程 + 今日线 + 逾期脉冲预警 + 车间负载 |
| 🧮 部门预算看板 | BudgetDashboardPage | 月度预算编制 + 执行率三色预警（费用审批口径） |
| ⚙️ 系统参数中心 | SysConfigPage | 成本费率/ABC阈值/信用开关统一管理，保存即时生效 |
| ⏱️ 审批时效报表 | ApprovalEfficiencyPage | 节点耗时统计 + 超48小时任务红色预警进待办 |
| 🗓️ 经营月报 | MonthlyReportPage | 销售/采购/库存/财务/质量五板块月报，打印+邮件订阅 |
...
- **业务流与财务流实时联动**：单据保存即在事务内自动生成凭证、应收应付、库存流水；
  列表行「🔗 业财」按钮一屏查看该单据的全部联动财务记录（凭证分录/应收应付/出入库/批次）
- **每个业务列表页自带汇总卡片 + 分析图**：20 张核心表内置聚合配置，近 6 月趋势 / TOP8 分布一键生成
- **批次全链路追溯**：采购批次 → 生产领料（FIFO）→ 生产批次成分 → 销售流向，
  正向/反向双向追溯，支持一键召回定位
- **行业物料编码规则开箱即用**：电子智造 12 类编码规则（FG-/IC-/MOD-/PCB-/PWR-…），
  商品新增时 ⚡ 一键生成防重编码
- **顶栏全局搜索**：功能页 + 162 张数据表即搜即达；仪表盘 KPI 一键穿透到对应数据表
- **账龄分析 + 报表打印**：应收应付账龄分桶催收看板；三大财务报表一键打印归档
- **经营工具箱（v5.12）**：销售送货单/采购订单套打（金额大写）、经营日报、
  凭证审核流（待审核→已审核→已记账）、商品/客户 Excel 批量导入（模板下载+幂等）
- **库存盘点与信用风控（v5.25）**：整仓批量盘点（账面→实盘→差异自动调账→盘盈亏凭证
  借1405/贷1901）；客户信用中枢——销售下单与报价转单双入口前置拦截超额订单，
  下单表单实时显示额度/占用/在途，管理员强制放行留审计痕
- **经营分析与薪酬引擎（v5.26）**：盘点盈亏分析报表（月度/仓库/商品三维聚合）；
  采购比价与供应商推荐（历史价对比 + 潜在节省测算）；销售提成阶梯计提
  （<60%→1.0% / ≥100%→2.5%+超额加计）审批后自动联动工资表
- **合同与质量闭环（v5.27）**：合同执行跟踪（订单挂接合同号，执行/回款进度 + 30 天到期
  待办提醒）；质量异常闭环 NCR（状态机管控 + 批次冻结 + 报废出库追溯）
- **供应链与经营深化（v5.28）**：供应商记分卡（交期/质量/价格三维加权评级）；
  合同收款计划与开票管理（分期收款 + 逾期催收待办 + 收款流水联动）；
  质量成本（COQ）联动毛利页；移动端快捷工作台（快捷盘点/出入库）
- **风控与智能决策（v5.29）**：催收管理中心（账龄四桶 + 催款函打印）；工单成本分析与损益；
  经营预测（双模型 + 目标联动）；敏感数据全链路脱敏（列表/导出，管理员明文开关）
- **全景运营（v5.30）**：客户 360 视图（六维聚合一键穿透）；生产排程甘特看板（今日线 + 逾期预警 + 车间负载）；
  库存 ABC 分类管理；部门预算编制与执行率三色预警
- **可配置化引擎（v5.31）**：系统参数配置中心（成本费率/ABC 阈值/信用开关，保存即时生效）；
  产品利润排行榜（贡献度 + 帕累托核心利润线）；供应商交付绩效明细；
  报表订阅扩展盘点盈亏/工单成本/应收账龄三类定时邮件
- **深度运营洞察（v5.32）**：库龄分析（批次持有天数分桶 + 呆滞风险联动）；
  审批时效报表（超 48 小时任务预警进待办）；客户复购分析（复购率/购买间隔）；
  经营月报（五大板块一键生成 + 打印 + 邮件订阅）
